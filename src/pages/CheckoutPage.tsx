...
const CheckoutPage = () => {
  ...
  const handleRazorpay = () => {
    ...
    script.onload = async () => {
      let rzpKey = '';
      try { const keyRes = await fetch('/api/razorpay-key'); const keyData = await keyRes.json(); rzpKey = keyData.key || ''; } catch { rzpKey = ''; }
      if (!rzpKey) { sonnerToast.error('Razorpay not configured', { description: 'Add RAZORPAY_KEY_ID to your Vercel environment variables.' }); setLoading(false); return; }
      const options = {
        key: rzpKey, amount: total * 100, currency: 'INR', name: 'Youtupia',
      ...
      rzp.on('payment.failed', (resp: any) => { sonnerToast.error('Payment failed', { description: resp.error?.description || 'Please try again.' }); setLoading(false); });
      rzp.open(); setLoading(false);
    };
    script.onerror = () => { sonnerToast.error('Could not load Razorpay. Check your internet connection.'); setLoading(false); };
  };
  ...
  const applyDiscount = () => {
    ...
    if (result.valid) {
      ...
      setDiscountApplied(true);
      setDiscountError('');
      const label = result.type === 'percentage' ? `${result.pct}% off` : `₹${result.amount} off`;
      sonnerToast.success(`${discountCode} applied — ${label}!`);
    } else {
  ...
  const inp = (key: string, label: string, type = 'text', ph = '', half = false) => (
    <div style={{ gridColumn: half ? 'span 1' : 'span 2' }}>
      ...
      <input
        type={type} value={(form as any)[key]} placeholder={ph}
          ...
          <div style={cardStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              <label onClick={() => setPaymentMethod('razorpay')}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <CreditCard size={15} style={{ color: '#ff0000' }} /> Pay Online — UPI / Card / Net Banking
                  </div>
                  <div style={{ fontSize: '12px', color: 'hsl(var(--muted-foreground))', marginTop: '3px' }}>Secured by Razorpay · All major cards, UPI, wallets accepted</div>
                </div>
              ...
              <label onClick={() => setPaymentMethod('cod')}
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontWeight: 700, fontSize: '14px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Banknote size={15} style={{ color: '#22c55e' }} /> Cash on Delivery (COD)
                  </div>
            ...
            {paymentMethod === 'cod' && (
              <div style={{ marginTop: '14px', padding: '12px 14px', background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.2)', borderRadius: '10px', display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                ...
                <div style={{ fontSize: '12px', color: 'hsl(var(--muted-foreground))', lineHeight: 1.6 }}>
                  <strong style={{ color: 'hsl(var(--foreground))' }}>COD note:</strong> Please keep exact change ready. COD orders cannot be cancelled after dispatch.
                </div>
              </div>
            )}
          </div>
        ...
        <div style={{ position: 'sticky', top: '24px' }}>
          <div style={cardStyle}>
            ...
            <div style={{ borderTop: '1px solid hsl(var(--border))', paddingTop: '14px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', gap: '7px' }}>
                      <input
                        ...
                        style={{ flex: 1, padding: '8px 11px', background: 'hsl(var(--secondary))', border: `1px solid ${discountError ? '#f87171' : 'hsl(var(--border))'}`, borderRadius: '7px', color: 'hsl(var(--foreground))', fontSize: '13px', outline: 'none', fontFamily: 'monospace' }}
                      />
                      <button onClick={applyDiscount} style={{ background: '#ff0000', color: 'white', border: 'none', borderRadius: '7px', padding: '8px 13px', cursor: 'pointer', fontSize: '12px', fontWeight: 700, fontFamily: 'Roboto, sans-serif', flexShrink: 0 }}>Apply</button>
                    </div>
                    {discountError && <div style={{ fontSize: '11px', color: '#f87171', marginTop: '5px' }}>{discountError}</div>}
                  </div>
                ) : (
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 11px', background: 'rgba(34,197,94,0.08)', border: '1px solid rgba(34,197,94,0.2)', borderRadius: '8px', fontSize: '13px' }}>
                    <span style={{ color: '#22c55e', fontWeight: 600 }}>✓ {discountCode} ({discountLabel})</span>
                    <button onClick={() => { setDiscountApplied(false); setDiscountPct(0); setDiscountFixed(0); setDiscountCode(''); setDiscountError(''); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'hsl(var(--muted-foreground))', fontSize: '11px', textDecoration: 'underline', fontFamily: 'Roboto, sans-serif' }}>Remove</button>
                  </div>
                )}
              </div>
              ...
              {paymentMethod === 'cod' && <div style={{ fontSize: '11px', color: 'hsl(var(--muted-foreground))', textAlign: 'right' }}>Pay on delivery</div>}
            </div>
            ...
            <div style={{ marginTop: '18px' }}>
              {paymentMethod === 'razorpay' ? (
                <button onClick={handleRazorpay} disabled={loading}
                  ...
                  {loading ? (<><span style={{ width: '16px', height: '16px', border: '2px solid white', borderTopColor: 'transparent', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.7s linear infinite' }} /> Processing...</>) : (<><Shield size={16} /> Pay ₹{total.toLocaleString()} Securely</>)}
                </button>
              ) : (
                <button onClick={handleCOD} disabled={loading}
                  ...
                  {loading ? (<><span style={{ width: '16px', height: '16px', border: '2px solid white', borderTopColor: 'transparent', borderRadius: '50%', display: 'inline-block', animation: 'spin 0.7s linear infinite' }} /> Placing Order...</>) : (<><Banknote size={16} /> Place Order — Pay on Delivery</>)}
                </button>
              )}
