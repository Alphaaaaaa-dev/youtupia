...
const useDynamicStats = () => {
  ...
  useEffect(() => {
    fetch('/api/orders')
      .then(r => r.json())
      .then(data => {
        const all = (data.orders || []).filter((o: any) => o.status !== 'cancelled');
        const itemsSold = all.reduce((s: number, o: any) => {
          const items = Array.isArray(o.items) ? o.items : [];
          return s + items.reduce((ss: number, i: any) => ss + (i.quantity || 1), 0);
        }, 0);
        const customers = new Set(all.map((o: any) => o.customerEmail).filter(Boolean)).size;
        setLiveStats({ itemsSold, customers });
      })
      .catch(() => {
        const itemsSold = orders.reduce((s, o) => s + o.items.reduce((ss, i) => ss + i.quantity, 0), 0);
        const customers = new Set(orders.map(o => o.customerEmail).filter(Boolean)).size;
        setLiveStats({ itemsSold, customers });
      });
  }, []);
    ...
    <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
      {[['d', time.d], ['h', time.h], ['m', time.m], ['s', time.s]].map(([label, val]) => (
        <div key={label} style={{ textAlign: 'center' }}>
          ...
          <div style={{ fontSize: 9, color: 'hsl(var(--muted-foreground))', marginTop: 4, textTransform: 'uppercase', letterSpacing: '0.12em' }}>{label}</div>
        </div>
      ))}
    </div>
  );
...
const ProductSkeleton = () => (
  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(210px,1fr))', gap: 16 }}>
    {Array.from({ length: 4 }).map((_, i) => (
      <div key={i} style={{ borderRadius: 12, overflow: 'hidden', background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))' }}>
        <div className="shimmer" style={{ aspectRatio: '3/4' }} />
        <div style={{ padding: '12px 12px 14px' }}>
          <div className="shimmer" style={{ height: 10, width: '40%', borderRadius: 6, marginBottom: 8 }} />
          <div className="shimmer" style={{ height: 14, width: '80%', borderRadius: 6, marginBottom: 8 }} />
          <div className="shimmer" style={{ height: 14, width: '30%', borderRadius: 6 }} />
        </div>
      </div>
    ))}
  </div>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '60px 24px', width: '100%' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 420px), 1fr))', gap: '48px', alignItems: 'center' }}>
            ...
            <div className="page-enter">
              ...
              <h1 style={{ fontSize: 'clamp(36px, 5.5vw, 68px)', fontWeight: 900, lineHeight: 1.02, letterSpacing: '-0.03em', marginBottom: '6px' }}>Wear you</h1>
              ...
              {latestDrop && (
                <div style={{ background: isDark ? 'rgba(255,0,0,0.06)' : 'rgba(255,0,0,0.05)', border: '1px solid rgba(255,0,0,0.16)', borderRadius: 18, padding: 18, marginBottom: 36 }}>
                  ...
                  {latestDrop.endsAt ? <DropCountdown endsAt={latestDrop.endsAt} /> : <div style={{ fontSize: 13, color: 'hsl(var(--muted-foreground))', fontWeight: 600 }}>Drop is live now.</div>}
                  <div style={{ display: 'flex', gap: 12, marginTop: 14, flexWrap: 'wrap' }}>
                    ...
                    <Link to="/drops" className="btn-ghost" style={{ textDecoration: 'none', borderRadius: 12, padding: '12px 18px', fontSize: 14 }}>View all drops</Link>
                  </div>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
                ...
                <Link to="/catalogue" className="btn-ghost" style={{ borderRadius: '10px', padding: '14px 28px', fontSize: '15px', textDecoration: 'none' }}>Browse Catalogue</Link>
              </div>
    ...
    <div key={s.label}>
      ...
      <div style={{ fontSize: '12px', color: 'hsl(var(--muted-foreground))', marginTop: '4px', fontWeight: 500 }}>{s.label}</div>
            ...
            <div className="page-enter" style={{ display: 'flex', flexDirection: 'column', gap: '16px', width: '100%' }}>
              <div style={{ borderRadius: '24px', overflow: 'hidden', background: 'rgba(0,0,0,0.35)', aspectRatio: '16/9', position: 'relative', border: isDark ? '1.5px solid rgba(255,255,255,0.1)' : '1.5px solid rgba(0,0,0,0.08)', boxShadow: '0 24px 64px rgba(0,0,0,0.3)', width: '100%' }}>
                {homePromo.videoUrl ? (() => {
                  ...
                  if (ytId) {
                    return (
                      <iframe src={`https://www.youtube.com/embed/${ytId}?autoplay=1&mute=1&loop=1&playlist=${ytId}&controls=1&modestbranding=1&rel=0`} title={homePromo.title || 'Promo Video'} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowFullScreen style={{ width: '100%', height: '100%', border: 'none', display: 'block' }} />
                    );
                  }
                  if (isInsta) {
                    const m = url.match(/instagram\.com\/(p|reel)\/([A-Za-z0-9_-]+)/);
                    if (m) return <iframe src={`https://www.instagram.com/${m[1]}/${m[2]}/embed`} title="Promo" allowFullScreen style={{ width: '100%', height: '100%', border: 'none', display: 'block' }} />;
                  }
                  ...
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, rgba(255,0,0,0.08) 0%, rgba(0,0,0,0.6) 100%)', color: 'rgba(255,255,255,0.7)', gap: '12px' }}>
                    ...
                    <span style={{ fontSize: '14px', fontWeight: 600 }}>Promotion Video</span>
              ...
              {dbLoading ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 8 }}>
                  {Array.from({ length: 4 }).map((_, i) => (
                    <div key={i} className="shimmer" style={{ aspectRatio: '3/4', borderRadius: 10 }} />
                  ))}
                </div>
              ) : (
        ...
        <div style={{ position: 'absolute', bottom: '32px', left: '50%', transform: 'translateX(-50%)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', opacity: 0.5 }}>
          ...
          <div style={{ width: '1px', height: '32px', background: 'linear-gradient(hsl(var(--muted-foreground)), transparent)' }} />
        </div>
      </section>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '20px 24px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '0' }}>
          {[
            ...
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 20px', borderRight: i < 3 ? '1px solid hsl(var(--border))' : 'none' }}>
              <div style={{ width: '38px', height: '38px', borderRadius: '10px', background: 'rgba(255,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', padding: '64px 24px' }}>
        <div className="reveal" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '32px' }}>
          ...
          <Link to="/shop" className="hover-underline" style={{ fontSize: '13px', color: '#ff0000', textDecoration: 'none', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>View all series <ArrowRight size={14} /></Link>
        </div>
        {dbLoading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 16 }}>
            {Array.from({ length: 2 }).map((_, i) => <div key={i} className="shimmer" style={{ height: 220, borderRadius: 20 }} />)}
          </div>
        ) : (
              ...
              <Link key={s.id} to={`/shop?series=${s.id}`} className="img-zoom"
                ...
                <div style={{ position: 'relative', padding: '20px', display: 'flex', alignItems: 'center', gap: '14px', width: '100%' }}>
                  <div style={{ width: '46px', height: '46px', borderRadius: '50%', overflow: 'hidden', border: '2px solid rgba(255,255,255,0.35)', background: 'rgba(0,0,0,0.3)', flexShrink: 0 }}>
                    {s.logo && <img src={s.logo} alt={s.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />}
                  </div>
                  <div style={{ flex: 1 }}>
                    <div style={{ color: 'white', fontWeight: 800, fontSize: '16px' }}>{s.name}</div>
      ...
      <section style={{ background: isDark ? 'hsl(0 0% 6%)' : 'hsl(var(--secondary))' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '64px 24px' }}>
          <div className="reveal" style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: '32px' }}>
            ...
            <Link to="/shop" className="hover-underline" style={{ fontSize: '13px', color: '#ff0000', textDecoration: 'none', fontWeight: 600, display: 'flex', alignItems: 'center', gap: '4px', flexShrink: 0 }}>See all <ArrowRight size={14} /></Link>
          </div>
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px 64px' }}>
        ...
        {dbLoading ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 16 }}>
            {Array.from({ length: 2 }).map((_, i) => <div key={i} className="shimmer" style={{ height: 200, borderRadius: 20 }} />)}
          </div>
        ) : (
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', padding: '64px 24px' }}>
        ...
        <div className="stagger" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
          {[
            { icon: Zap, title: 'Limited Runs Only', desc: 'Every drop is limited. We never restock. Once it\'s gone, it\'s gone forever.' },
            { icon: Package, title: 'Obsessed With Quality', desc: 'Minimum 240gsm cotton for tees, 380gsm fleece for hoodies.' },
            { icon: Users, title: 'Community First', desc: 'Every drop is informed by the community. You vote, we drop.' },
            { icon: Star, title: 'Premium Every Time', desc: 'From packaging to the final stitch — every detail is deliberate.' },
      ...
      <section style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px 64px' }}>
        <div className="reveal" style={{ background: 'linear-gradient(135deg, #ff0000 0%, #cc0000 100%)', borderRadius: '24px', padding: '56px 48px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '32px', flexWrap: 'wrap', position: 'relative', overflow: 'hidden' }}>
          ...
          <Link to="/shop" style={{ background: 'white', color: '#ff0000', textDecoration: 'none', padding: '14px 28px', borderRadius: '12px', fontWeight: 800, fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0, boxShadow: '0 4px 20px rgba(0,0,0,0.2)' }}
      ...
      <footer style={{ borderTop: '1px solid hsl(var(--border))', background: isDark ? 'hsl(0 0% 5%)' : 'hsl(var(--secondary))' }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '48px 24px 32px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '40px' }}>
          <div>
            ...
            <p style={{ fontSize: '13px', color: 'hsl(var(--muted-foreground))', lineHeight: 1.8, maxWidth: '240px', marginBottom: '20px' }}>Premium merch for the culture. Limited drops, real quality.</p>
          </div>
          {[
            { heading: 'Shop', links: [['All Products', '/shop'], ['Drops', '/drops'], ['Wishlist', '/wishlist'], ['New Arrivals', '/shop']] },
            { heading: 'Info', links: [['About Us', '/about'], ['Track Order', '/track-order'], ['FAQ', '/faq'], ['Contact', '/contact'], ['Policies', '/policy']] },
            { heading: 'Connect', links: [['📧 youtupiastore@gmail.com', '/contact'], ['📸 @_youtupia_', '/contact'], ['💬 Contact Us', '/contact']] },
          ].map(col => (
            <div key={col.heading}>
              <div style={{ fontWeight: 700, fontSize: '13px', marginBottom: '16px' }}>{col.heading}</div>
              {col.links.map(([label, to]) => (
                <div key={label} style={{ marginBottom: '10px' }}>
                  <Link to={to} style={{ fontSize: '13px', color: 'hsl(var(--muted-foreground))', textDecoration: 'none' }}
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '20px 24px', borderTop: '1px solid hsl(var(--border))', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '20px' }}>
            {[['Privacy', '/policy'], ['Terms', '/policy'], ['Refunds', '/policy']].map(([l, to]) => (
              <Link key={l} to={to} style={{ fontSize: '12px', color: 'hsl(var(--muted-foreground))', textDecoration: 'none' }}>{l}</Link>
            ))}
          </div>
        </div>
      </footer>
